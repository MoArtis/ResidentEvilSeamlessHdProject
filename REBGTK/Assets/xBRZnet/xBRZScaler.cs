...
namespace xBRZNet
{
    ...
    */
    ...
    {
        public Color32[] ScaleImage(Texture2D texture, int scaleSize, ScalerCfg config = null)
        {
            ...
            Bitmap result = ScaleImage(bitmap, scaleSize, config);

            Color32[] colors = new Color32[result.Width * result.Height];
            for (int y = 0; y < result.Height; y++)
            {
                for (int x = 0; x < result.Width; x++)
                {
                    System.Drawing.Color c = result.GetPixel(x, y);
                    colors[(result.Height - y - 1) * result.Width + x] = new Color32(c.R, c.G, c.B, c.A);
                }
            }
            return colors;
        }
