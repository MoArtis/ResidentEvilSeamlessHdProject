...
namespace RE2
{
    public class RdtParser
    {
        ...
        public bool ParseRdtData(byte[] data, string filename, out RdtRoom outRoom)
        {
            ...
            room.stage = filename.Substring(4, 1);
            room.room = filename.Substring(5, 2);
            room.player = filename.Substring(7, 1);
            room.name = string.Concat("ROOM_", room.stage, room.room, "_P", room.player);
